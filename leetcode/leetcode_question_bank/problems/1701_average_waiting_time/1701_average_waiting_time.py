...
once he is idle. The customer waits till the chef finishes preparing his order. The
...
class Solution:
    ...
    def averageWaitingTime(self, customers: List[List[int]]) -> float:
        ...
        for arrival_time, time_needed in customers:
            current_time = max(arrival_time, current_time)
            total_time += time_needed + (current_time - arrival_time)
...
if __name__ == '__main__':
    ...
    test(solution.averageWaitingTime([[1,2],[2,5],[4,3]]), 5.00000)
    test(solution.averageWaitingTime([[5,2],[5,4],[10,3],[20,1]]), 3.25000)
