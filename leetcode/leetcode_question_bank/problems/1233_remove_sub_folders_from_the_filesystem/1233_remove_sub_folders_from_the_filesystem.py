...
of folder[j] must start with folder[j], followed by a "/". For example, "/a/b" is a sub-folder
of "/a", but "/b" is not a sub-folder of "/a/b/c".
...
class Solution:
    ...
    def removeSubfolders(self, folder: List[str]) -> List[str]:
        ...
        for f in folder:
            if len(previous) > 0 and f.startswith(previous) and f[len(previous)] == "/":
                continue
            result.append(f)
            previous = f
        return result
...
if __name__ == "__main__":
    ...
    test(solution.removeSubfolders(["/a","/a/b","/c/d","/c/d/e","/c/f"]), ["/a","/c/d","/c/f"])
    test(solution.removeSubfolders(["/a","/a/b/c","/a/b/d"]), ["/a"])
    test(solution.removeSubfolders(["/a/b/c","/a/b/ca","/a/b/d"]), ["/a/b/c","/a/b/ca","/a/b/d"])
