...
class Solution:
    ...
    def passThePillow(self, n: int, time: int) -> int:
        ...
        while (time > 0):
            if direction == 1 and position < n:
                position += direction
                time -= 1
            elif direction == 1 and position == n:
                direction = -1
            elif direction == -1 and position > 1:
                position += direction
                time -= 1
            elif direction == -1 and position == 1:
                direction = 1
...
if __name__ == '__main__':
    ...
    test(solution.passThePillow(4, 5), 2)
    test(solution.passThePillow(3, 2), 3)
    test(solution.passThePillow(18, 38), 5)
