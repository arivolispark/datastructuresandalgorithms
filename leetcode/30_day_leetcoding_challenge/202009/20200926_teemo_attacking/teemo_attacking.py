...
2) You may assume the numbers in the Teemo's attacking time series and his poisoning time duration per
...
class Solution:
    ...
    def findPoisonedDuration(self, timeSeries: List[int], duration: int) -> int:
        ...
        last_start_time = 0
        last_poisoned_time = 0

        timeSeries.append(20000000)

        for start_time in timeSeries:
            if start_time > last_poisoned_time:
                total_poisoned_duration += last_poisoned_time - last_start_time
                last_start_time = start_time

            last_poisoned_time = start_time + duration
...
if __name__ == "__main__":
    ...
    test(solution.findPoisonedDuration([1,4], 2), 4)
    test(solution.findPoisonedDuration([1,2], 2), 3)
