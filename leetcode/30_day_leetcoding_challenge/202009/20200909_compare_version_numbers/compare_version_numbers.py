    ...
    def compareVersion(self, version1: str, version2: str) -> int:
        ...
        if version1_list:
            for i in range(len(version1_list)):
                l1.append(int(version1_list[i]))

        if version2_list:
            for i in range(len(version2_list)):
                l2.append(int(version2_list[i]))
...
if __name__ == "__main__":
    ...
    test(solution.compareVersion("0.1", "1.1"), -1)
    test(solution.compareVersion("1.0.1", "1"), 1)
    test(solution.compareVersion("7.5.2.4", "7.5.3"), -1)
    test(solution.compareVersion("1.01", "1.001"), 0)
    test(solution.compareVersion("1.0", "1.0.0"), 0)
